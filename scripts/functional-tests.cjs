...
test('curriculum IDs, pathway references, questions, and answer keys are internally valid', () => {
  ...
  assert.equal(lessons.length, 153);
  assert.equal(sqeLessons.length, 138);
  assert.equal(sqeQuestions.length, 1092);
...
test('SQE stage and station inventory stays consistent (not a completeness claim)', () => {
  assert.deepEqual(sqeTotals, {
    flk1Subjects: 7,
    flk2Subjects: 7,
    sqe2Skills: 6,
    lessons: 138,
    practiceQuestions: 1092,
    sqe2Stations: 24,
  });
...
test('multilingual navigation uses semantic forward and back directions', () => {
  ...
  assert.doesNotMatch(i18n, /code: '(ar|zh|es)'/);
  for (const code of ['en']) assert.match(i18n, new RegExp(`code: '${code}'[^\\n]*rtl: false`));

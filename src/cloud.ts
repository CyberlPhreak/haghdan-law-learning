...
export type CloudSnapshot = {
  ...
  progress: {
    ...
    study_progress?: LearnerState['studyProgress'];
...
export async function fetchCloudSnapshot(userId: string): Promise<CloudSnapshot> {
  ...
  const [profile, settings, progress] = await Promise.all([
    ...
    supabase.from('learner_progress').select('xp, completed_lessons, saved_lessons, quiz_scores, study_progress, completion_dates, review_queue, active_days, test_history, updated_at').eq('user_id', userId).maybeSingle(),
  ]);
...
export async function syncCloudState(state: LearnerState, user: User) {
  ...
  const results = await Promise.all([
    ...
    supabase.from('learner_progress').upsert({
      ...
      study_progress: state.studyProgress,

...
import { authoredUnits, authoredSubject } from './authored-content';
import { allocateBlueprintCounts, firstSessionTargets } from './sqe-spec';
...
const seeds: Seed[] = [
  ...
  },
  {
    ...
    units:[
      ...
      'contract-restitution~داراشدن ناموجه و استرداد~Unjust enrichment and restitution~enrichment، at the claimant expense، unjust factors، failure of basis و change of position',
    ],
  },
  {
...
export const sqeLessons: Lesson[] = [
  ...
  return authored ? { ...lesson, sections: authored.sections.fa, quiz: authored.questions.fa, contentVersion: authored.version } : lesson;
});
...
export const sqeQuestions: SqeQuestion[] = [...Object.values(authoredUnits).flatMap(unit => unit.questions.fa.map(question => ({ ...question, stage: 'FLK1' as const, subjectId: authoredSubject[unit.id]!, unitId: unit.id }))), ...seeds.filter(subject => subject.stage==='FLK1'||subject.stage==='FLK2').flatMap(subject =>
  subject.units.filter(raw => row(raw).id !== 'contract-restitution').flatMap(raw => Array.from({ length: 12 }, (_, variant) => makeQuestion(subject, raw, variant))),
)];
export const questionsForStage = (stage: SqeStage) => sqeQuestions.filter(question => question.stage === stage);

/** A distinct practice pool: never pad authored exercises with generated variants. */
export const buildAuthoredPracticeQuestions = (stage: SqeStage, count: number, subjectId?: string) =>
  shuffleQuestions(questionsForStage(stage).filter(question => question.id.startsWith('authored-') && (!subjectId || question.subjectId === subjectId)))
    .slice(0, Number.isFinite(count) ? Math.max(0, Math.floor(count)) : 0);
...
/** Two separately shuffled sessions; session one cannot leak into session two. */
export const buildFullMockQuestions = (stage: SqeStage) => {
  const selected = buildBalancedTestQuestions(stage, 180);
  const firstIds = new Set(Object.entries(firstSessionTargets[stage]).flatMap(([subjectId, count]) =>
    selected.filter(question => question.subjectId === subjectId).slice(0, count).map(question => question.id)));
  const first = selected.filter(question => firstIds.has(question.id));
  const second = selected.filter(question => !firstIds.has(question.id));
  if (first.length !== 90 || second.length !== 90) throw new Error('Insufficient question coverage for two 90-question sessions.');
  return [...shuffleQuestions(first), ...shuffleQuestions(second)];
};

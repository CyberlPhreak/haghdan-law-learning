...
import { acknowledgeStudySection, normalizeStudyProgress, type StudyProgress } from './study-progress';
...
export type TestAttempt = { id: string; stage: 'FLK1' | 'FLK2'; mode: TestMode; score: number; correct: number; total: number; completedAt: string; durationSeconds: number; subjectScores?: Record<string, SubjectScore>; questionResults?: Array<{ questionId: string; selectedIndex: number | null; correctIndex: number; contentVersion: string }> };
...
type StoreValue = {
  ...
  acknowledgeReading: (lessonId: string, section: number) => void;
...
const hasProgress = (state: LearnerState) => (
  ...
  || Object.keys(state.studyProgress).length > 0
);
...
const stateFromCloud = (
  ...
  return {
    ...
    studyProgress: normalizeStudyProgress(progress?.study_progress),
...
export function LearnerProvider({ children }: { children: ReactNode }) {
    ...
    AsyncStorage.getItem(STORAGE_KEY)
      .then((raw) => {
        ...
        const restored: LearnerState = {
          ...
          studyProgress: normalizeStudyProgress(saved.studyProgress),
  ...
  const acknowledgeReading = useCallback((lessonId: string, section: number) => {
    const lesson = lessonById[lessonId];
    if (!lesson) return;
    setState(current => ({
      ...current,
      studyProgress: acknowledgeStudySection(current.studyProgress, lessonId, lesson.contentVersion ?? 'summary-v1', section, lesson.sections.length),
      activeDays: [...new Set([...current.activeDays, todayKey()])],
    }));
  }, []);
  ...
  const updateSettings = useCallback((patch: SettingsPatch) => {
    setState((current) => ({ ...current, ...patch, language: normalizeLanguage(patch.language ?? current.language) }));
  }, []);
  ...
  const value = useMemo<StoreValue>(
    ...
    [state, cloud, registerAccount, authenticate, signInWithGoogle, resendVerification, sendPasswordReset, updatePassword, startGuest, completeTutorial, restartTutorial, signOut, completeLesson, acknowledgeReading, reviewAnswer, toggleSaved, updateSettings, recordTestAttempt, resetProgress, deleteAccount, streak, completedToday],
  );

...
export function buildLearningAnalytics(state: LearnerState, locale = 'fa-IR'): LearningAnalytics {
  ...
  const readingRecords = Object.entries(state.studyProgress ?? {}).filter(([id, record]) => lessons.some(lesson => lesson.id === id && record.version === (lesson.contentVersion ?? 'summary-v1'))).map(([, record]) => record);
  ...
  return {
    ...
    readingCompleted: readingRecords.filter(record => record.completedAt !== null).length,
    readingStarted: readingRecords.length,

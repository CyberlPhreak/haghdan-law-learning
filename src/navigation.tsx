...
import { elapsedExamSeconds, remainingExamSeconds } from './exam-time';
...
import { studyResumeSection } from './study-progress';
import { authoredUnits } from './authored-content';
...
import { buildAuthoredPracticeQuestions, buildBalancedTestQuestions, buildFullMockQuestions, sqeTotals, sqeQuestions, stageSubjects, type SqeStage, type SqeTrack } from './sqe';
...
export type RootStackParamList = {
  ...
  Test: { stage: SqeStage; count: number; mode: TestMode; subjectId?: string; authoredOnly?: boolean };
  ...
  return (
    <NavigationContainer theme={{ ...DefaultTheme, dark: darkMode, colors: { ...DefaultTheme.colors, primary: palette.primary, background: palette.background, card: palette.surface, text: palette.ink, border: palette.line } }}>
      <Root.Navigator screenOptions={{ headerShown: false, animation: isRtl ? 'slide_from_left' : 'slide_from_right', contentStyle: { backgroundColor: palette.background } }}>
        ...
        <Root.Screen name="TestReview" component={TestReviewScreen} />
  ...
  return (
    <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.detailPage}>
      ...
      <View style={s.list}>{item.lessonIds.map((id, index) => {
        ...
        const record = state.studyProgress[id];
        const done = Boolean(record?.completedAt && record.version === (lesson.contentVersion ?? 'summary-v1'));
...
function LessonScreen({ route, navigation }: LessonProps) {
  ...
  const { state, completeLesson, acknowledgeReading, toggleSaved } = useLearner();
  const { t, formatNumber, legalTitle, language } = useI18n();
  const { width } = useWindowDimensions();
  ...
  const [section, setSection] = useState(() => studyResumeSection(state.studyProgress[sourceItem?.id ?? ''], sourceItem?.contentVersion ?? 'summary-v1', sourceItem?.sections.length ?? 0));
  ...
  useEffect(() => { readingScroll.current?.scrollTo({ y: 0, animated: false }); }, [mode, section, quiz, studyComplete, finished, language]);
  if (!item) return null;
  ...
  const advance = () => {
    if (mode === 'study') {
      acknowledgeReading(item.id, section);
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView ref={readingScroll} contentContainerStyle={[s.lessonPage, compact && s.lessonPageCompact]}>
      ...
      {mode === 'study' && !studyComplete ? <View style={s.lessonFooter}><Text style={s.hint}>{language === 'fa' ? 'پیشرفت مطالعه ذخیره می‌شود؛ نمره تمرین جداست.' : 'Reading progress is saved separately from practice scores.'}</Text>{section > 0 ? <ActionButton label={t('common.back')} direction="back" variant="secondary" onPress={() => setSection(section - 1)} fullWidth /> : null}<ActionButton label={section === item.sections.length - 1 ? t('lesson.finishStudy') : t('lesson.nextSection')} direction="forward" onPress={advance} sound fullWidth /></View> : null}
...
function LessonSection({ item, index, compact }: { item: Lesson; index: number; compact: boolean }) {
  ...
  const authored = authoredUnits[item.id];
  return <View style={[s.reading, compact && s.readingCompact]}>
    ...
    {index === 0 && authored ? <View style={s.checklistCard}><Text style={s.checklistTitle}>{language === 'fa' ? 'هدف‌های یادگیری' : 'Learning objectives'}</Text>{authored.objectives[language].map(objective => <Text key={objective} style={s.checkText}>• {objective}</Text>)}</View> : null}
    ...
    <View style={s.term}><Text style={s.hint}>{t('lesson.keyTerm')}</Text><Text selectable style={s.termFa}>{language === 'fa' ? part.termFa : part.termEn}</Text>{language === 'fa' ? <Text selectable style={s.english}>{part.termEn}</Text> : null}</View>
    {authored ? <View style={s.checklistCard}><Text style={s.checklistTitle}>{language === 'fa' ? 'منابع و وضعیت تحریریه' : 'Sources and editorial status'}</Text><Text style={s.hint}>{language === 'fa' ? 'نسخه دوزبانه تألیفی؛ پیش‌نویس نیازمند بازبینی حقوقی مستقل.' : 'Authored bilingual edition. Editorial draft awaiting independent legal review.'}</Text>{authored.sources.map(source => <Pressable key={source.url} accessibilityRole="link" onPress={() => { void Linking.openURL(source.url).catch(() => Alert.alert(language === 'fa' ? 'پیوند باز نشد' : 'Could not open link', source.url)); }} style={{ minHeight: 48, justifyContent: 'center' }}><Text style={s.checkText}>{source.title} ↗</Text></Pressable>)}</View> : null}
...
function TestScreen({ route, navigation }: TestProps) {
  const { stage, count, mode, subjectId, authoredOnly } = route.params;
  const { state, recordTestAttempt } = useLearner();
  const { playMilestone } = useSoundFeedback();
  const { t, formatNumber, legalTitle, language } = useI18n();
  const sourceQuestions = useMemo(() => authoredOnly ? buildAuthoredPracticeQuestions(stage, count, subjectId) : mode === 'fullMock' ? buildFullMockQuestions(stage) : buildBalancedTestQuestions(stage, count, subjectId), [stage, count, subjectId, authoredOnly, mode]);
  ...
  const [earnedXp, setEarnedXp] = useState(0);
  const deadline = useRef(Date.now() + duration * 1000);
  const priorSessionSeconds = useRef(0);
  const question = questions[index]!;
  const answered = Object.keys(answers).length;
  const currentAnswered = !!question && answers[question.id] !== undefined;
  ...
  const finishTest = () => {
    ...
    recordTestAttempt({ stage, mode, score: percent, correct: right, total: questions.length, durationSeconds: priorSessionSeconds.current + elapsedExamSeconds(duration, remainingExamSeconds(deadline.current, Date.now())), subjectScores,
      questionResults: sourceQuestions.map(question => ({ questionId: question.id, selectedIndex: answers[question.id] ?? null, correctIndex: question.correctIndex, contentVersion: question.id.startsWith('authored-') ? authoredUnits[question.unitId]?.version ?? 'legacy-v1' : 'legacy-v1' })),
    });
    playMilestone();
    void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  };
  ...
  useEffect(() => {
    ...
    const updateClock = () => setRemaining(remainingExamSeconds(deadline.current, Date.now()));
    updateClock();
    const timer = setInterval(updateClock, 1000);
    const subscription = AppState.addEventListener('change', next => { if (next === 'active') updateClock(); });
  ...
  const startSecondSession = () => {
    priorSessionSeconds.current = elapsedExamSeconds(duration, remaining);
    deadline.current = Date.now() + duration * 1000;
    setIndex(90); setRemaining(duration); setInBreak(false);
  };

  if (!question) return <SafeAreaView style={s.safe}><TopBar onBack={navigation.goBack} /><Empty icon="book-open" title={language === 'fa' ? 'پرسش در دست تهیه است' : 'Questions are being prepared'} body={language === 'fa' ? 'برای این مبحث هنوز پرسش تألیفی موجود نیست؛ پرسش تکراری جایگزین نمی‌کنیم.' : 'No authored questions are available for this selection yet. We do not fill the gap with repeated variants.'} /></SafeAreaView>;
...
function Review() {
  ...
  const [selected, setSelected] = useState<number | null>(null);
  const [revealed, setRevealed] = useState(false);
  // An archived question must not hide all the valid cards behind it after a content update.
  const availableReviews = state.reviewQueue.filter(entry => lessonById[entry.lessonId]?.quiz.some(question => question.id === entry.questionId));
  const due = availableReviews.filter((entry) => new Date(entry.dueAt) <= new Date());
  const record = due[0];
...
function Practice() {
  ...
  return <Page tone="practice">
    ...
    <View style={s.examNote}><Feather name="alert-circle" size={20} color={palette.primary} /><View style={s.flexEnd}><Text style={s.smallStrong}>{language === 'fa' ? 'بانک پرسش هنوز کامل و بازبینی‌شده نیست' : 'Question bank: incomplete editorial draft'}</Text><Text style={s.hint}>{language === 'fa' ? '۲۴ پرسش سناریویی جدید برای قرارداد و مسئولیت مدنی فراهم شده است. بانک قدیمی هنوز شامل گونه‌های تولیدشده و نیازمند جایگزینی است. نمره این تمرین‌ها پیش‌بینی قبولی یا نتیجه رسمی SRA نیست.' : '24 new scenario questions cover contract and tort. The older bank still contains generated variants needing replacement. These practice scores are not a pass prediction or an official SRA result.'}</Text></View></View>
    <ActionButton label={language === 'fa' ? 'تمرین سناریوهای جدید · ۱۰ پرسش' : 'Practise new scenarios · 10 questions'} icon="book-open" onPress={() => nav.navigate('Test', { stage: 'FLK1', count: 10, mode: 'quick', authoredOnly: true })} fullWidth />
    ...
    {state.testHistory.filter(attempt => attempt.questionResults?.length).slice(0, 6).map(attempt => <ActionButton key={`review-${attempt.id}`} label={`${language === 'fa' ? 'مرور پاسخ‌ها' : 'Review answers'} · ${attempt.stage} · ${new Date(attempt.completedAt).toLocaleString(locale)}`} icon="list" variant="secondary" onPress={() => nav.navigate('TestReview', { attemptId: attempt.id })} fullWidth />)}
...
function TestReviewScreen({ route, navigation }: NativeStackScreenProps<RootStackParamList, 'TestReview'>) {
  const { state } = useLearner();
  const { language, t, formatNumber } = useI18n();
  const attempt = state.testHistory.find(item => item.id === route.params.attemptId);
  const [index, setIndex] = useState(0);
  const result = attempt?.questionResults?.[index];
  const source = result ? sqeQuestions.find(question => question.id === result.questionId) : undefined;
  const currentVersion = source?.id.startsWith('authored-') ? authoredUnits[source.unitId]?.version : 'legacy-v1';
  const available = source && result && currentVersion === result.contentVersion && source.correctIndex === result.correctIndex;
  const question = available ? localizeQuestion(source, language) : undefined;
  const total = attempt?.questionResults?.length ?? 0;
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.detailPage}>
    <TopBar onBack={navigation.goBack} />
    <Text style={s.pathHeroTitle}>{language === 'fa' ? 'مرور پاسخ‌های آزمون' : 'Test answer review'}</Text>
    <Text style={s.hint}>{formatNumber(index + (total ? 1 : 0))} / {formatNumber(total)} · {attempt?.stage} · {language === 'fa' ? 'مرور نمره و امتیاز شما را تغییر نمی‌دهد.' : 'Review does not change your score or award XP.'}</Text>
    {question && result ? <><Text style={s.hint}>{result.selectedIndex === null ? (language === 'fa' ? 'بی‌پاسخ' : 'Not answered') : (language === 'fa' ? 'پاسخ ثبت‌شده شما در زیر مشخص است.' : 'Your recorded selection is shown below.')}</Text><QuizCard question={question} selected={result.selectedIndex} revealed onSelect={() => undefined} /></> : <Text style={s.readingBody}>{language === 'fa' ? 'جزئیات این تلاش قدیمی موجود نیست یا نسخه پرسش تغییر کرده است. نمره اصلی حفظ شده است.' : 'Details are unavailable for this older attempt, or the question version has changed. Your original score is preserved.'}</Text>}
    <ActionButton label={t('common.back')} direction="back" variant="secondary" disabled={index === 0} onPress={() => setIndex(value => value - 1)} fullWidth />
    <ActionButton label={t('lesson.nextQuestion')} direction="forward" disabled={index + 1 >= total} onPress={() => setIndex(value => value + 1)} fullWidth />
  </ScrollView></SafeAreaView>;
}
...
function InsightsScreen({ navigation }: InsightsProps) {
  ...
  const { t, formatNumber, legalTitle, locale, language } = useI18n();
  const analytics = useMemo(() => buildLearningAnalytics(state, locale), [state, locale]);
      ...
      <ScrollView contentContainerStyle={[s.detailPage, s.insightsPage]}>
        ...
        <MotionView style={s.insightsMotion} distance={14} duration={motion.relaxed}>
          ...
          <View style={s.insightMetrics}>
            <InsightMetric icon="book" value={formatNumber(analytics.readingCompleted)} label={language === 'fa' ? 'درس مطالعه‌شده' : 'Lessons read'} color={palette.teal} soft={palette.tealSoft} />
            <InsightMetric icon="bookmark" value={formatNumber(analytics.readingStarted - analytics.readingCompleted)} label={language === 'fa' ? 'مطالعه در جریان' : 'Reading in progress'} color={palette.primary} soft={palette.primarySoft} />
            ...
            <InsightMetric icon="clock" value={formatNumber(analytics.studyMinutes)} label={language === 'fa' ? 'زمان پیشنهادی درس‌های تمرین‌شده (دقیقه)' : 'Estimated lesson time (minutes), not tracked time'} color={palette.teal} soft={palette.tealSoft} />
